// Gyros oldal JavaScript - JAVÍTOTT VERZIÓ (Árfrissítés nélkül)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addToCartButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // ELLENŐRZÉS: Ha desszert, akkor nincs speciális kérés
            let specialRequest = '';
            const isDessert = itemName.includes('Torta') || itemName.includes('Tiramisu') || itemName.includes('Baklava');
            
            if (!isDessert) {
                // Csak nem desszert esetében keressük a textarea-t
                const textarea = menuItem.querySelector('textarea');
                if (textarea) {
                    specialRequest = textarea.value;
                }
            }
            
            // Opciók összegyűjtése (csak gyrosoknál)
            const options = getSelectedOptions(menuItem);
            
            // Item név opciókkal (csak gyrosoknál)
            let fullItemName = itemName;
            if (!isDessert && options.meat) {
                fullItemName += ` (${options.meat})`;
            }
            if (!isDessert && options.sauce) {
            ...
            const cartItem = {
                id: itemId,
                name: fullItemName,
                price: basePrice,        // CSAK EGYSÉGÁR (1 db gyros)
                quantity: quantity,      // MENNYISÉG
                specialRequest: specialRequest,  // Desszertnél üres
                image: imageUrl,
                type: isDessert ? 'desszert' : 'gyros'
            };
            
            // Csak gyrosoknál adjuk hozzá az opciókat
            if (!isDessert) {
            ...
            // Speciális kérés mező törlése (csak ha van és nem desszert)
            if (!isDessert) {
                const textarea = menuItem.querySelector('textarea');
                if (textarea) {
                    textarea.value = '';
            ...
            // Opciók visszaállítása alapértelmezettre (csak gyrosoknál)
            if (!isDessert) {
    ...
    addDrinkButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const cartItem = {
                id: itemId,
                name: itemName,
                price: basePrice,        // CSAK EGYSÉGÁR (1 db ital)
                quantity: quantity,      // MENNYISÉG
                image: imageUrl,
                type: 'drink'
            };
    ...
    // Opciók változás eseménykezelő - CSAK LOGOLJUK (nem változtatja az árat)
    // A funkció meghívása eltávolítva, mert nincs ár változás
    
    // Az ár frissítés függvényt teljesen eltávolítottuk, mert nem kell
});