// Kosár oldal JavaScript - JAVÍTOTT (Utólagos kedvezmény, Max 2 db azonos típusból)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Termékek normál és akciós árai
    // Itt állítjuk be az ÚJ eredeti árakat (1600, 2700, 3800)
    const productPrices = {
        'Pita Gyros': { normal: 1600, discount: 1300 }, // Megtakarítás: 300 Ft
        'Kis Gyros Tál': { normal: 2700, discount: 2000 }, // Megtakarítás: 700 Ft
        'Nagy Gyros Tál': { normal: 3800, discount: 2600 } // Megtakarítás: 1200 Ft
    };
    ...
    // JAVÍTVA: Mindig az eredeti árat mutatja, nincs "AKCIÓS" kiírás
    function createCartItemElement(item, index) {
        ...
        // így felülírjuk, ha a localStorage-ba más került volna.
        ...
        // Termék név összeállítása - SIMA név, nincs AKCIÓS előtag
    ...
    // Mennyiség növelése
    // JAVÍTVA: Kivettem a "Max 1 db akciós" korlátot. Bármennyit lehet rendelni, a kedvezmény majd a végén számolódik.
    ...
    // Összesítő frissítése
    // JAVÍTVA: Itt számoljuk a kedvezményt a "Max 2 db azonos típusú gyros" szabály szerint
    function updateSummary() {
        let subtotal = 0;
        let discountTotal = 0;
        
        // Változók a kedvezmény logikához
        let discountedItemName = null; // Melyik termékre aktiválódott a kedvezmény (Pita vagy Tál)
        let discountedCount = 0;       // Hány darabnál járunk (max 2 lehet)
        ...
        // 2. Kedvezmény számítása (Utólagos levonás)
        // Sorban megyünk a tételeken. Amelyik először jön és Gyros, az "stoppolja" a kedvezményt.
        cartItems.forEach(item => {
            const baseName = item.name.split('(')[0].split(' - ')[0].trim();
            
            // Ha ez egy Gyros termék (van benne az árlistában)
            if (productPrices[baseName]) {
                // Ha még nincs kiválasztva akciós termék, akkor EZ lesz az (az első amit találunk)
                if (discountedItemName === null) {
                    discountedItemName = baseName;
                }
                
                // Ha ez a tétel UGYANAZ a termék, mint amit kiválasztottunk (pl. Pita), akkor jár rá kedvezmény
                if (baseName === discountedItemName) {
                    const quantity = parseInt(item.quantity || 1);
                    const normalPrice = productPrices[baseName].normal;
                    const discountPrice = productPrices[baseName].discount;
                    const savingPerItem = normalPrice - discountPrice;
                    
                    // Kiszámoljuk hány darabra érvényesíthető még a kedvezmény (Max 2 összesen)
                    const remainingSlots = 2 - discountedCount;
                    
                    if (remainingSlots > 0) {
                        // Vagy a teljes mennyiség, vagy amennyi még hátravan a limitből
...
Email: info@tonygyros.hu`;
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', async function(e) {
            ...
            // 6. Összegek újraszámolása a biztonság kedvéért (visszakapjuk az objectet)
            ...
            const orderData = {
                ...
                discountTotal: summaryData.discountTotal, // Hozzáadva a kedvezmény
